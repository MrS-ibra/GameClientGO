...
namespace {
	...
	const Real BRIGHTNESS_BOOST = 0.1f;   // Apply a slight brightness to make darker colors more visible
}
...
static UnicodeString formatPowerAction(const AsciiString& powerNameAscii)
{
	...
	if (powerNameAscii == "Infa_SuperweaponInfantryParadrop") return L"DEPLOYED AN INF PARADROP!";
	if (powerNameAscii == "Tank_SuperweaponTankParadrop") return L"DEPLOYED A TANK PARADROP!";
	if (powerNameAscii == "SuperweaponParadropAmerica") return L"DEPLOYED A USA PARADROP!";
	if (powerNameAscii == "SuperweaponLeafletDrop") return L"DEPLOYED A LEAFLET DROP";
	...
	if (powerNameAscii == "SuperweaponClusterMines" || powerNameAscii == "Nuke_SuperweaponClusterMines") return L"CALLED IN THE MINES!";
	...
	if (powerNameAscii == "SuperweaponCIAIntelligence") return L"ACTIVATED THE CIA INTELLIGENCE!";
	if (powerNameAscii == "Slth_SuperweaponGPSScrambler" || powerNameAscii == "SuperweaponGPSScrambler") return L"ACTIVATED A GPS SCRAMBLER!";
	if (powerNameAscii == "SuperweaponSneakAttack") return L"ACTIVATED A SNEAK ATTACK!!!";
...
void InGameUI::drawObserverNotifications(Int& x, Int& y)
{
	...
	Int baseX = Int(20 * scale);
	Int baseY = (screenH / 2) - Int(300 * scale);
	Int padX = Int(12 * scale);
	Int padY = Int(10 * scale);
	Int boxSpacing = Int(8 * scale);

	Color bgColor = TheWindowManager->winMakeColor(0, 0, 0, 150);
...
void InGameUI::updateObserverNotifications(UnsignedInt currentFrame)
{
	if (m_observerMilestones.empty()) {
		...
		for (Int i = 0; i < MAX_SLOTS; ++i)
			m_observerMilestones[i] = ObserverMilestone{ i };
	}
...
void InGameUI::checkObserverMilestones(UnsignedInt currentFrame)
{
	for (Int slotIndex = 0; slotIndex < MAX_SLOTS; ++slotIndex) {
		...
		if (cash >= 100000 && !milestone.warnedFloating20k) {
			milestone.warnedFloating20k = true;
...
void InGameUI::addObserverNotificationRaw(const UnicodeString& message, Color color)
{
	...
	for (auto& n : m_observerNotifications)
		if (n.createdRenderMs < oldest->createdRenderMs)
			oldest = &n;
...
{
	if (!player || !powerTemplate || player->isPlayerObserver())
		return;
...
void InGameUI::refreshObserverStatsResources(void)
{
	...
	m_observerStatsLineStep = statsFont ? statsFont->height + 2 : adjustedFontSize + 2;
}
